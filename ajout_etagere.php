...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        if (isset($_POST['stock_ordinateur'])) {
            ...
            $sql = "INSERT INTO ordinateurs_etagere (marque, modele, ram, disque_dur, graphique, accessoire, prixAchat, prixVente) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if (isset($_POST['stock_autres'])) {
            ...
            $sql = "INSERT INTO autres_materiels_etagere (marque, description, prixAchat, prixVente) VALUES (?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
    ...
    header("Location: etagere.php?message=$message");
    exit();
}
?>
